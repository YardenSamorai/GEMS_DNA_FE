...
function Header({ total }) {
  return (
    <section className="glass-surface rounded-2xl sm:rounded-[28px] px-6 sm:px-9 py-7 sm:py-9 flex items-end justify-between gap-6 flex-wrap">
      <div className="max-w-xl">
        ...
        <p className="text-[14px] sm:text-[15px] text-glass-muted mt-4 max-w-md leading-relaxed">
          Every memo you have received with at least one electronic signature. Each record can be exported as a print-ready PDF.
        </p>
      </div>
      <div className="text-[12px] text-glass-muted font-medium tabular-nums">
...
function Toolbar({ search, onSearch, statusFilter, onStatusFilter }) {
  ...
  return (
    <div className="px-5 sm:px-7 py-4 sm:py-5 border-b border-glass-line/70 flex items-center gap-4 flex-wrap">
...
function DocumentRow({ memo, busy, onDownload, onPrint }) {
  ...
  return (
    <div className="flex items-start gap-4 sm:gap-5 px-5 sm:px-7 py-5 hover:bg-white/45 transition-colors">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          {memo.signature_count > 0 && (
            <span className="inline-flex items-center text-[10.5px] font-medium px-2 py-0.5 rounded-full bg-portal-champagne/12 text-portal-champagne2 tabular-nums">
              {memo.signature_count} signature{memo.signature_count === 1 ? "" : "s"}
            </span>
          )}
          {!memo.has_sig_issue_supplier && (
            <span className="inline-flex items-center text-[10.5px] font-medium px-2 py-0.5 rounded-full bg-rose-50/70 text-rose-700 border border-rose-100">
              Awaiting supplier
            </span>
          )}
        </div>
        <Link
          ...
          className="block text-glass-ink text-[16px] sm:text-[17px] font-semibold mt-2 break-all hover:text-black transition-colors tracking-tight"
        >
          {memo.memo_number}
        </Link>
      ...
      <div className="shrink-0 flex flex-col sm:flex-row items-end sm:items-center gap-2">
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full text-[12.5px] font-medium bg-white/65 border border-white/70 backdrop-blur-md text-glass-ink hover:bg-white/85 transition-colors shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] disabled:opacity-40 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full text-[12.5px] font-medium bg-glass-ink text-white hover:bg-glass-graphite transition-colors shadow-[0_4px_14px_-6px_rgba(20,22,28,0.45)] disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function SignatureBadges({ memo }) {
  const Badge = ({ ok, label }) => (
    <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium ${ok ? "text-portal-champagne2" : "text-glass-soft"}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${ok ? "bg-portal-champagne" : "bg-glass-line2"}`} />
      {label}
    </span>
  );
...
function ListSkeleton() {
  return (
    <div className="divide-y divide-glass-line/70 animate-pulse">
      {[0, 1, 2].map((i) => (
        <div key={i} className="px-5 sm:px-7 py-5 flex items-center gap-5">
          ...
          <div className="h-8 w-28 rounded-full bg-glass-canvas2" />
        </div>
      ))}
    </div>
  );
