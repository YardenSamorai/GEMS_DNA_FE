      ...
      <div className="w-full sm:w-[560px] bg-white shadow-2xl flex flex-col h-full overflow-hidden">
        {loading || !contact ? (
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="px-4 sm:px-5 pt-3 pb-4 border-b border-stone-200 space-y-3">
              {/* Row 1 — action bar skeleton */}
          ...
          <>
            {/* ─────────────────────────────────────────────────────────
                Header — redesigned for clarity (Apr 2026):
                  Row 1: compact action bar (Close · label · View profile · Edit · Delete)
                  Row 2: identity block (avatar + name + title + company)
                  Row 3: meta strip (Type · Location · Folder · Website)
                  Row 4: dedicated Assignee row (only when team has reps)
                  Row 5: tags
                  Row 6: quick actions (Call / WhatsApp / Email)
                Mobile and desktop share the same structure — only paddings
                & button sizes differ. The previous header crammed avatar,
                name, badge, picker AND four icons into one row, which broke
                under any drawer narrower than ~600px and rendered the
                AssigneePicker twice.
            ──────────────────────────────────────────────────────── */}
            <div
            ...
            >
              ...
              <div className="flex items-center gap-1.5 mb-3">
                <button
                  onClick={onClose}
                  title="Close"
                  className="p-1.5 -ml-1.5 rounded-lg text-stone-600 hover:bg-stone-100"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                  </svg>
                </button>
                <div className="flex-1 text-[11px] font-semibold uppercase tracking-wider text-stone-400 truncate">
                  Contact
                </div>
                <Link
                  to={`/crm/customers/${contact.id}`}
                  title="View full customer profile"
                  className="hidden sm:inline-flex items-center gap-1 px-2.5 h-8 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 text-xs font-semibold hover:bg-emerald-100"
                >
                  View full profile
              ...
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-stone-700 to-stone-900 text-white flex items-center justify-center font-bold text-lg sm:text-xl shrink-0">
                  {(contact.name || "?").charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1 pt-0.5">
                  <h2 className="text-lg sm:text-xl font-bold text-stone-900 leading-tight break-words">
                    {contact.name}
                  </h2>
                  {contact.title && (
                    <div className="text-xs sm:text-sm text-stone-600 font-medium mt-0.5 break-words">
                      {contact.title}
                    </div>
                  )}
                  {contact.company && (
                    <div className="text-xs sm:text-sm text-stone-500 mt-0.5 break-words">
                      {contact.company}
                    </div>
                  )}
                </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5">
                ...
                {(contact.city || contact.country) && (
                  <span className="text-xs text-stone-500 inline-flex items-center gap-1 min-w-0">
                ...
                {contact.folder_name && (
                  <span className="text-xs text-stone-500 inline-flex items-center gap-1 min-w-0">
                    <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 7a2 2 0 012-2h4l2 2h7a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" />
                    </svg>
                    <span className="truncate">{contact.folder_name}</span>
                  </span>
                )}
                {normaliseWebsite(contact.website) && (
                  <a
                    href={normaliseWebsite(contact.website)}
                    target="_blank" rel="noreferrer"
                    className="text-xs text-blue-600 hover:underline inline-flex items-center gap-1 min-w-0 max-w-full"
                  >
                    <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {/* Row 4 — dedicated assignee row (single picker, no duplicate) */}
              {team.ready && team.members.length > 1 && (
                <div className="mt-3 flex items-center gap-2">
                  <span className="text-[11px] font-semibold uppercase tracking-wider text-stone-400 shrink-0">
                    Assigned to
                  </span>
                  <AssigneePicker
              ...
              {/* Row 6 — quick actions (only render the channels we actually have) */}
              {(contact.phone || contact.email) && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {contact.phone && (
                    <a
                      href={`tel:${contact.phone}`}
                      className="flex-1 min-w-[90px] flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-stone-100 text-stone-700 text-xs font-semibold hover:bg-stone-200 active:scale-[0.98] transition"
                    >
                  ...
                  {contact.phone && (
                    <a
                      ...
                      target="_blank" rel="noreferrer"
                      className="flex-1 min-w-[90px] flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-emerald-100 text-emerald-700 text-xs font-semibold hover:bg-emerald-200 active:scale-[0.98] transition"
                    >
                  ...
                  {contact.email && (
                    <a
                      ...
                      className="flex-1 min-w-[90px] flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-blue-100 text-blue-700 text-xs font-semibold hover:bg-blue-200 active:scale-[0.98] transition"
                    >
                      <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {/* Row 7 — mobile-only "view full profile" (desktop has it in the top action bar) */}
              <Link
                to={`/crm/customers/${contact.id}`}
                className="sm:hidden mt-3 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-semibold hover:bg-emerald-100 border border-emerald-200"
              >
                View full profile
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
